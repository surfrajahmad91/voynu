...
export function validateCapacity({ passengerCount, luggageCount, passengerCapacity, luggageCapacity }) {
  ...
  const maxLuggage = Number(luggageCapacity);
  ...
  if (!Number.isInteger(maxLuggage) || luggage > maxLuggage) {
    return { valid: false, reason: `This vehicle can accommodate up to ${maxLuggage} luggage items.` };
  }
