...
const CURRENT_FLEET_MAX_LUGGAGE = 4;
 ...
 * The current fleet tops out at 4 luggage spaces and luggage is capped at
 * 3 items per passenger.
 */
export function getMaxLuggageForPassengers(passengerCount, absoluteLimit = CURRENT_FLEET_MAX_LUGGAGE) {
  ...
  return Math.min(
    ...
    CURRENT_FLEET_MAX_LUGGAGE,
    Number.isFinite(Number(absoluteLimit)) ? Number(absoluteLimit) : CURRENT_FLEET_MAX_LUGGAGE
  );
}

/**
 * Passenger count has priority over luggage capacity.
 * A vehicle may use unused passenger capacity for luggage, while its normal
 * luggage capacity remains available. Luggage is always capped at 3 per
 * passenger.
 *
 ...
 * - SUV (6 pax / 4 bags): 4 pax + 4 bags and 5 pax + 4 bags are valid.
 */
export function getMaxLuggageForVehicle({ passengerCount, passengerCapacity, luggageCapacity }) {
  ...
  return Math.min(capacityBasedLuggage, passengers * MAX_LUGGAGE_PER_PASSENGER);
}
