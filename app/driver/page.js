...
export default function DriverPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (cancelled) return;
      if (driverError || !driverRow || driverRow.active === false) { setNotADriver(true); setChecking(false); return; }
      setDriver(driverRow); setChecking(false);
    };
    load();
  ...
  const activeTrips = bookings.filter((b) => b.booking_status !== "trip_completed" && b.booking_status !== "cancelled");
  ...
  return <main style={{ minHeight: "100vh", background: theme.colors.bg, fontFamily: theme.fontFamily, color: theme.colors.text }}>
    ...
    <div style={{ width: `min(${theme.maxWidth.content}px, calc(100% - 32px))`, margin: "0 auto", padding: "24px 0 60px" }}>
      ...
      <h2 style={{ fontSize: 15, fontWeight: 800, margin: "0 0 12px" }}>Your trips</h2>
      {loadingBookings ? <p style={{ color: theme.colors.textFaint, fontSize: 13 }}>Loading...</p> : activeTrips.length === 0 ? <p style={{ color: theme.colors.textFaint, fontSize: 13 }}>No trips assigned right now.</p> : <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>{activeTrips.map((b) => { const step = NEXT_STATUS[b.booking_status]; const status = statusColors[b.booking_status] || statusColors.driver_assigned; return <div key={b.id} style={{ padding: "16px 18px", borderRadius: theme.radius.lg, background: theme.colors.surface, border: `1px solid ${theme.colors.border}`, boxShadow: theme.shadow.card }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}><span style={{ fontSize: 13, fontWeight: 700 }}>{b.travel_date} · {b.pickup_time}</span><span style={{ padding: "4px 10px", borderRadius: 20, fontSize: 10.5, fontWeight: 700, textTransform: "capitalize", background: status.bg, color: status.text }}>{(b.booking_status || "").replace(/_/g, " ")}</span></div><div style={{ fontSize: 13.5, fontWeight: 600, lineHeight: 1.6, marginBottom: 10 }}>📍 {b.pickup_name}<br />🏁 {b.drop_name}</div><div style={{ fontSize: 12, color: theme.colors.textMuted, marginBottom: 12 }}>Passenger: <strong>{b.passenger_name}</strong> · {b.phone}<br />{b.trip_type === "roundtrip" ? "Round Trip" : "One Way"} · {b.vehicle_type} · ₹{b.fare} · {b.payment_method}</div>{step && <button onClick={() => handleAdvanceStatus(b)} style={{ width: "100%", minHeight: 46, border: 0, borderRadius: 12, background: theme.gradients.primary, color: "#ffffff", fontFamily: theme.fontFamily, fontWeight: 800, fontSize: 13.5, cursor: "pointer" }}>{step.label}</button>}</div>; })}</div>}
      {pastTrips.length > 0 && <><h2 style={{ fontSize: 15, fontWeight: 800, margin: "24px 0 12px" }}>Completed</h2><div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{pastTrips.map((b) => <div key={b.id} style={{ padding: "12px 16px", borderRadius: 12, background: "#ffffff", border: `1px solid ${theme.colors.border}`, fontSize: 12.5, color: theme.colors.textMuted }}>{b.travel_date} · {b.pickup_name} → {b.drop_name} · ₹{b.fare}</div>)}</div></>}
    </div>
  </main>;
}
