...
export default function DispatchPage() {
  ...
  const load = async () => {
    ...
    const [{ data: setting, error: se }, { data: bs, error: be }, { data: ds, error: de }] = await Promise.all([
      ...
      supabase.from("bookings").select("id,booking_status,payment_status,travel_date,pickup_time,pickup_name,drop_name,vehicle_type,passenger_count,luggage_count,driver_id,vehicle_category_id").eq("booking_status", "confirmed").is("driver_id", null).order("travel_date").order("pickup_time"),
  ...
  const setDispatchMode = async (next) => {
    ...
    const { error: e } = await supabase.from("dispatch_settings").update({ mode: next, updated_at: new Date().toISOString() }).eq("id", true);
    setBusy(false);
    if (e) return setError(e.message);
    setMode(next);
    setMessage(`Automatic dispatch is ${next === "automatic" ? "ON" : "OFF"}.`);
    await load();
  };
  ...
  return <main style={{ minHeight: "100vh", background: "#f4f6f5", color: "#1d2b24", fontFamily: "ui-monospace,monospace", padding: 18 }}>
    <div style={{ maxWidth: 1000, margin: "0 auto" }}>
      ...
      <section style={{ background: "#fff", border: "1px solid #d9e0dc", borderRadius: 9, padding: 16, marginBottom: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}><div><strong>Dispatch mode</strong><div style={{ color: "#66756d", fontSize: 11, marginTop: 5 }}>{mode === "automatic" ? "Eligible confirmed bookings will be assigned automatically." : "Bookings remain awaiting assignment until an admin assigns a driver."}</div></div><div style={{ display: "flex", gap: 7 }}><button disabled={busy} onClick={() => setDispatchMode("manual")} style={mode === "manual" ? primary : button}>MANUAL</button><button disabled={busy} onClick={() => setDispatchMode("automatic")} style={mode === "automatic" ? primary : button}>AUTOMATIC</button></div></div>
      </section>
