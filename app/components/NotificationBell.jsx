...
export default function NotificationBell({ targetPath = "/account", audience = "customer" }) {
  ...
  const allowedTypes = NOTIFICATION_TYPES[audience] || NOTIFICATION_TYPES.customer;
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`voynu-notifications-${userId}-${audience}`)
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "notifications", filter: `user_id=eq.${userId}` },
        (payload) => {
          if (allowedTypes.includes(payload.new?.type)) {
