...
export default function DateTimeBookingGuard() {
  useEffect(() => {
    ...
    const fields = () => ({ dates: Array.from(document.querySelectorAll('input[type="date"]')), times: Array.from(document.querySelectorAll('input[type="time"]')) });
    ...
    const ensureStyle = () => {
      ...
      style.textContent = `
        input[data-voynu-datetime-enhanced] { box-sizing:border-box!important; width:100%!important; min-height:62px!important; padding:0 18px 0 48px!important; border:1.5px solid #d9e7df!important; border-radius:18px!important; background-color:#f9fcfa!important; color:#24352d!important; font-size:18px!important; font-weight:650!important; letter-spacing:.1px!important; box-shadow:0 1px 2px rgba(18,70,43,.04),inset 0 0 0 1px rgba(255,255,255,.65)!important; transition:border-color .18s ease,box-shadow .18s ease,background-color .18s ease!important; }
        input[data-voynu-datetime-enhanced]:focus { border-color:#08783f!important; background-color:#fff!important; box-shadow:0 0 0 4px rgba(8,120,63,.10)!important; outline:none!important; }
        input[data-voynu-datetime-enhanced][data-voynu-datetime-invalid="true"] { border-color:#d55a4d!important; background-color:#fff9f8!important; box-shadow:0 0 0 3px rgba(213,90,77,.08)!important; }
        input[data-voynu-datetime-enhanced]::-webkit-calendar-picker-indicator { opacity:.62; cursor:pointer; width:22px; height:22px; }
        [data-voynu-datetime-message] { box-sizing:border-box; margin:8px 2px 0!important; padding:10px 12px!important; border-radius:12px!important; font-size:13px!important; line-height:1.35!important; font-weight:650!important; }
        [data-voynu-datetime-message="error"] { color:#a93429!important; background:#fff2f0!important; border:1px solid #f0cbc6!important; }
        [data-voynu-datetime-message="hint"] { color:#617168!important; background:transparent!important; border:0!important; padding:7px 2px 0!important; font-weight:500!important; }
      `;
    ...
    const validate = () => {
      ...
      else { clearMessage(dates[0]); addHint(dates[0], travelDate === today ? "Today" : "Choose the day you want to travel"); }
      ...
      if (travelDate === today) {
        ...
        } else if (pickupTime < now) { addMessage(times[0], "This pickup time has already passed. Please choose a later time today."); valid = false; }
        else { clearMessage(times[0]); addHint(times[0], "Pickup time"); }
      } else {
        times[0].removeAttribute("min"); clearMessage(times[0]); addHint(times[0], travelDate ? "Pickup time" : "Select a travel date first");
      }
      ...
      if (returnDate && travelDate && returnDate !== travelDate) { addMessage(dates[1], "Round trips return on the same day. Please select the same date as your pickup."); valid = false; }
      else if (!returnDate) {
      ...
      } else { clearMessage(dates[1]); addHint(dates[1], "Same-day return"); }
      ...
      if (travelDate && returnDate === travelDate && pickupTime) {
        ...
        if (start && durationSeconds != null) {
          ...
          if (!returnTime) {
            ...
            else { clearMessage(times[1]); addHint(times[1], `Estimated arrival ${minReturn} • Latest return ${maxReturn} • Waiting fee set in VOYNU Admin`); }
          }
        } else {
          times[1].removeAttribute("min"); times[1].removeAttribute("max"); clearMessage(times[1]); addHint(times[1], "Return time will be limited after route duration is calculated");
        }
      } else if (returnDate === travelDate && returnTime && pickupTime) {
        times[1].removeAttribute("min"); times[1].removeAttribute("max");
        if (returnTime <= pickupTime) { addMessage(times[1], "Return time must be later than pickup time."); valid = false; } else clearMessage(times[1]);
