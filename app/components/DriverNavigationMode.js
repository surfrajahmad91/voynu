...
export default function DriverNavigationMode({ booking, driverLocation, targetType = "destination", onExit, onComplete }) {
  ...
  useEffect(() => {
    const measure = () => setWidth(Math.max(300, window.innerWidth));
    measure();
  ...
  const mapHeight = Math.max(420, Math.round(window?.innerHeight || 700));
  ...
  const navTitle = targetType === "pickup" ? "Navigate to passenger" : "Navigate to destination";
  ...
  const eta = route?.durationSeconds ? formatDuration(route.durationSeconds) : "—";

  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 9999, width: "100vw", height: "100dvh", minHeight: "100vh", background: "#eef5f0", fontFamily: "Arial, sans-serif", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: "max(12px, env(safe-area-inset-top))", left: 12, right: 12, display: "flex", justifyContent: "space-between", gap: 10, zIndex: 10, pointerEvents: "none" }}>
