...
function dbForUser(accessToken) {
  ...
  return createClient(supabaseUrl, anonKey, { auth: { autoRefreshToken: false, persistSession: false }, global: { headers: { Authorization: `Bearer ${accessToken}` } } });
}
...
export async function POST(request) {
  try {
    ...
    // Phase 5: never trust a distance or fare supplied by the browser. Recalculate
    // the authoritative road distance on the server at booking confirmation time.
    let roadDistance;
    try {
      roadDistance = await getRoadDistance(booking.pickup, booking.drop);
    } catch (distanceError) {
      console.error("VOYNU authoritative booking distance failed:", distanceError);
      return NextResponse.json({ error: distanceError?.message || "We could not verify the road distance for this journey. Please try again.", code: "AUTHORITATIVE_DISTANCE_FAILED", stage: "road_distance.authoritative" }, { status: 502 });
    }
    const oneWayKm = roadDistance.distanceKm;
    ...
    const row = {
      ...
      fare_breakdown: {
        ...
        authoritativeOneWayDistanceKm: oneWayKm,
        authoritativeDistanceText: roadDistance.distanceText,
      },
    ...
    return NextResponse.json({ error: "Unable to create booking right now. Please try again.", code: "BOOKING_CREATE_UNHANDLED_ERROR", stage: "bookings.create.unhandled" }, { status: 500 });
  }
}
