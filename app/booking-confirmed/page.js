  ...
  if (!loaded || !booking) {
    return (
      <main style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: theme.colors.bg }}>
        <div style={{ width: 34, height: 34, border: "3px solid rgba(8,120,63,0.18)", borderTopColor: theme.colors.primary, borderRadius: "50%" }} />
      </main>
    );
  }
      ...
      <div style={{ width: `min(${theme.maxWidth.content}px, calc(100% - 32px))`, margin: "0 auto", padding: "32px 0 60px" }}>
        ...
        <div style={{ padding: 20, borderRadius: theme.radius.lg, background: theme.colors.surface, border: `1px solid ${theme.colors.border}`, boxShadow: theme.shadow.card }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 11 }}>
            ...
            <div style={{ fontSize: 13.5, fontWeight: 600, color: "#24352b", lineHeight: 1.4 }}>{booking.pickup?.name}</div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: 11 }}>
            ...
            <div style={{ fontSize: 13.5, fontWeight: 600, color: "#24352b", lineHeight: 1.4 }}>{booking.drop?.name}</div>
          </div>
          ...
          {isRoundTrip && (
            <div style={{ marginTop: 14, paddingTop: 14, borderTop: `1px dashed ${theme.colors.border}`, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <DetailCell label="Return date" value={booking.returnDate} />
              <DetailCell label="Return time" value={booking.returnTime} />
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginTop: 22, padding: "16px 18px", borderRadius: theme.radius.lg, background: theme.colors.warningBg, border: "1px solid #f0dfa8" }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 800, color: theme.colors.warning }}>Need help before your journey?</div>
            <div style={{ marginTop: 3, fontSize: 12, lineHeight: 1.5, color: "#8a6b1c" }}>
              Your booking details are already with the VOYNU team. If you need to make a change, please contact us and quote your booking reference.
            </div>
          </div>
        </div>
