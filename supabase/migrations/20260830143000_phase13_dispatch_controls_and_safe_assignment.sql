...
-- Automatic dispatch is an explicit admin-controlled operation. We deliberately do
-- not use a booking trigger because driver assignment is an operational action and
-- must run through the authenticated admin path with the existing booking guards.
