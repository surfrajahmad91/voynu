-- VOYNU 2.0 / P0: make commute subscription creation idempotent.
-- Evidence: three identical subscriptions were created within ~2s on 2026-09-17,
-- and the wallet trigger debits on every INSERT, so a double submit double-debits.
-- 1) create_commute_subscription: per-user advisory lock + return existing matching
--    pending_payment request. 2) Unique partial index as a hard backstop.
-- 3) Revoke non-API table privileges (TRUNCATE/REFERENCES/TRIGGER) from anon/authenticated.
-- Safe to apply: no rows currently in pending_payment/active/paused.
...
begin
  if auth.role() <> 'service_role'
     and (auth.role() <> 'authenticated' or auth.uid() is null or auth.uid() is distinct from p_user_id) then
    raise exception 'VOYNU: subscription creation access denied';
  end if;
  ...
  -- Idempotency: serialise concurrent submits per customer, and return the
  -- already-created unpaid request instead of inserting (and wallet-debiting) again.
  perform pg_advisory_xact_lock(hashtextextended('commute_subscription_create:'||p_user_id::text,0));
  select * into v_subscription
  from public.commute_subscriptions s
  where s.user_id=p_user_id
    and s.status='pending_payment'
    and s.plan_id=v_plan_id
    and s.vehicle_category_id=p_vehicle_category_id
    and s.pickup_lat=p_pickup_lat and s.pickup_lon=p_pickup_lon
    and s.drop_lat=p_drop_lat and s.drop_lon=p_drop_lon
    and s.start_date=p_start_date
    and s.morning_pickup_time=p_morning_pickup_time
    and s.evening_return_time=p_evening_return_time
    and s.weekdays=p_weekdays
...
create unique index if not exists commute_subscriptions_open_request_uidx
on public.commute_subscriptions(
  user_id, plan_id, vehicle_category_id, pickup_lat, pickup_lon, drop_lat, drop_lon,
  start_date, morning_pickup_time, evening_return_time, weekdays
)
where status in ('pending_payment','active','paused');

revoke truncate, references, trigger on public.commute_subscriptions from anon, authenticated;
