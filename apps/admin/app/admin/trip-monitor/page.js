...
export default function TripMonitorPage() {
  ...
  const visible = useMemo(() => bookings.filter((b) => {
    if (filter === "attention") return alerts.some((a) => a.booking_id === b.id) || b.trip_start_on_time === false || b.return_trip_start_on_time === false || b.trip_completion_on_time === false || cashIssue(b);
