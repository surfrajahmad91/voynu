...
export default function TripMonitorPage() {
  ...
  const load = async (runWatchdog = true) => {
    ...
    const [{ data: bs, error: be }, { data: as, error: ae }] = await Promise.all([
      ...
      supabase.from("trip_timing_alerts").select("id,booking_id,alert_type,threshold_minutes,created_at,acknowledged_at").is("acknowledged_at", null).order("created_at", { ascending: false }),
    ]);
  ...
  const metrics = useMemo(() => {
    const now = Date.now();
    const notStarted = bookings.filter((b) => b.scheduled_pickup_at && !b.trip_started_at && !["trip_completed", "cancelled"].includes(b.booking_status) && new Date(b.scheduled_pickup_at).getTime() < now).length;
    const lateStarts = bookings.filter((b) => b.trip_start_on_time === false).length;
    const lateCompletions = bookings.filter((b) => b.trip_completion_on_time === false).length;
    const active = bookings.filter((b) => b.booking_status === "trip_started").length;
    return { notStarted, lateStarts, lateCompletions, active };
  }, [bookings]);

  const visible = useMemo(() => bookings.filter((b) => {
    if (filter === "attention") return alerts.some((a) => a.booking_id === b.id) || b.trip_start_on_time === false || b.trip_completion_on_time === false;
    if (filter === "active") return ["driver_assigned", "on_the_way", "arrived", "trip_started"].includes(b.booking_status);
    if (filter === "completed") return b.booking_status === "trip_completed";
    return true;
  }), [bookings, alerts, filter]);

  const acknowledge = async (alert) => {
    const { error: e } = await supabase.from("trip_timing_alerts").update({ acknowledged_at: new Date().toISOString() }).eq("id", alert.id);
  ...
  return <main style={{ minHeight: "100vh", background: theme.colors.bg, color: theme.colors.text, fontFamily: theme.fontFamily, padding: 18 }}>
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      <header style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", marginBottom: 18, flexWrap: "wrap" }}><div><h1 style={{ margin: 0, fontSize: 24 }}>Trip Monitor</h1><p style={{ margin: "5px 0 0", color: theme.colors.textFaint, fontSize: 12 }}>Track scheduled starts, actual starts, expected completion and late trips.</p></div><div style={{ display: "flex", gap: 8 }}><button onClick={() => load(true)} style={{ padding: "9px 14px", borderRadius: 10, border: 0, background: theme.colors.primary, color: "#fff", fontWeight: 800 }}>Run check</button><Link href="/admin" style={{ padding: "9px 14px", borderRadius: 10, border: `1px solid ${theme.colors.border}`, background: "#fff", color: theme.colors.text, textDecoration: "none", fontWeight: 800 }}>Admin</Link></div></header>
      ...
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12, marginBottom: 16 }}>
        {[['Not started on time',metrics.notStarted,theme.colors.warning],['Active trips',metrics.active,theme.colors.primary],['Late starts recorded',metrics.lateStarts,'#b36b00'],['Late completions recorded',metrics.lateCompletions,theme.colors.error]].map(([label,value,color]) => <div key={label} style={{ background: "#fff", border: `1px solid ${theme.colors.border}`, borderRadius: 14, padding: 16 }}><div style={{ fontSize: 11, color: theme.colors.textFaint, fontWeight: 800, textTransform: "uppercase" }}>{label}</div><div style={{ marginTop: 6, fontSize: 28, fontWeight: 900, color }}>{value}</div></div>)}
      </section>
      ...
      {visible.length === 0 ? <div style={{ padding: 28, background: "#fff", border: `1px solid ${theme.colors.border}`, borderRadius: 14, color: theme.colors.textFaint }}>No trips match this view.</div> : <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{visible.map((b) => { const related = alerts.filter((a) => a.booking_id === b.id); const startLate = b.trip_start_on_time === false; const completionLate = b.trip_completion_on_time === false; return <article key={b.id} style={{ background: "#fff", border: `1px solid ${related.length || startLate || completionLate ? "#e7b26a" : theme.colors.border}`, borderRadius: 14, padding: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}><div><strong style={{ fontSize: 15 }}>{ref(b.id)}</strong> · {b.passenger_name || "Passenger"}<div style={{ marginTop: 5, color: theme.colors.textMuted, fontSize: 12 }}>{b.pickup_name} → {b.drop_name}</div></div><span style={{ padding: "5px 10px", borderRadius: 20, background: theme.colors.primaryTint, color: theme.colors.primary, fontSize: 10, fontWeight: 900, textTransform: "uppercase" }}>{(b.booking_status || "").replace(/_/g," ")}</span></div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 10, marginTop: 14 }}>
          <div><div style={{ fontSize: 10, color: theme.colors.textFaint }}>SCHEDULED START</div><strong>{fmt(b.scheduled_pickup_at)}</strong></div>
          <div><div style={{ fontSize: 10, color: theme.colors.textFaint }}>ACTUAL START</div><strong>{fmt(b.trip_started_at)}</strong>{startLate && <div style={{ color: theme.colors.error, fontSize: 11, fontWeight: 800 }}>{b.trip_start_delay_minutes} min late</div>}</div>
          <div><div style={{ fontSize: 10, color: theme.colors.textFaint }}>EXPECTED COMPLETION</div><strong>{fmt(b.scheduled_completion_at)}</strong></div>
          <div><div style={{ fontSize: 10, color: theme.colors.textFaint }}>ACTUAL COMPLETION</div><strong>{fmt(b.completed_at)}</strong>{completionLate && <div style={{ color: theme.colors.error, fontSize: 11, fontWeight: 800 }}>{b.trip_completion_delay_minutes} min late</div>}</div>
        </div>
        {related.length > 0 && <div style={{ marginTop: 12, padding: 11, borderRadius: 10, background: "#fff7e8", color: "#8a5700", fontSize: 12, fontWeight: 800 }}>{related.map((a) => <div key={a.id} style={{ display: "flex", justifyContent: "space-between", gap: 8, marginBottom: 4 }}><span>{a.alert_type === "start_overdue" ? "Trip has not started after scheduled pickup." : "Trip has passed expected completion."}</span><button onClick={() => acknowledge(a)} style={{ border: 0, background: "transparent", color: theme.colors.primary, fontWeight: 900 }}>Acknowledge</button></div>)}</div>}
      </article>; })}</div>}
      {lastCheck && <div style={{ marginTop: 12, fontSize: 10, color: theme.colors.textFaint }}>Last timing check: {lastCheck.toLocaleTimeString("en-IN")}. The monitor checks again every minute while this page is open.</div>}
    </div>
  </main>;
}
