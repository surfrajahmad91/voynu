    ...
    <main style={{ minHeight: "100vh", background: theme.colors.bg, fontFamily: theme.fontFamily, color: theme.colors.text }}>
      ...
      <div style={{ width: `min(${theme.maxWidth.content}px, calc(100% - 32px))`, margin: "0 auto", padding: "32px 0 60px" }}>
        ...
        <div style={{ padding: 20, borderRadius: theme.radius.lg, background: theme.colors.surface, border: `1px solid ${theme.colors.border}`, boxShadow: theme.shadow.card }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 11 }}><div style={{ width: 10, height: 10, marginTop: 4, borderRadius: "50%", background: theme.colors.primary, flexShrink: 0 }} /><div style={{ fontSize: 13.5, fontWeight: 600, color: theme.colors.text, lineHeight: 1.4 }}>{booking.pickup?.name}</div></div>
          <div style={{ width: 1.5, height: 16, marginLeft: 4.25, background: theme.colors.border }} />
          <div style={{ display: "flex", alignItems: "flex-start", gap: 11 }}><div style={{ width: 10, height: 10, marginTop: 4, borderRadius: "50%", background: theme.colors.accent, flexShrink: 0 }} /><div style={{ fontSize: 13.5, fontWeight: 600, color: theme.colors.text, lineHeight: 1.4 }}>{booking.drop?.name}</div></div>
        ...
        <div style={{ marginTop: 14, padding: "13px 16px", borderRadius: theme.radius.lg, background: theme.colors.primaryTint, fontSize: 11.5, color: theme.colors.primaryDark, lineHeight: 1.5 }}>Free cancellation up to 1 hour before pickup. Cancelling closer to your pickup time applies a small fee to help compensate your driver. Manage or cancel this booking anytime from My Account.</div>
