...
export const metadata = { title:"VOYNU", description:"Book your perfect trip with VOYNU", manifest:"/manifest.webmanifest", icons:{ icon:"/voynu-lockup.svg", apple:"/voynu-lockup.svg" }, appleWebApp:{ capable:true, title:"VOYNU", statusBarStyle:"black-translucent" } };
...
export default function RootLayout({children}){return <html lang="en"><body><script dangerouslySetInnerHTML={{__html:themeInit}}/><DateTimeBookingGuard/>{children}<PushNotifications targetPath="/account" audience="customer"/><script dangerouslySetInnerHTML={{__html:`if("serviceWorker"in navigator){window.addEventListener("load",()=>navigator.serviceWorker.register("/sw.js").catch(()=>{}));}`}}/></body></html>}
