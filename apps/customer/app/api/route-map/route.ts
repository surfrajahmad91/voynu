...
export async function POST(request: Request) {
  try {
    ...
    try {
      response = await fetch(`${ROUTER_URL}/${originLon},${originLat};${destinationLon},${destinationLat}?overview=full&geometries=geojson&steps=true`, {
    ...
    if (!response.ok) {
      console.error("VOYNU routing service error:", response.status, data);
    ...
    console.error("VOYNU route-map error:", error);
