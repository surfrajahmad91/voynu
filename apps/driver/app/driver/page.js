...
export default function DriverPage() {
  ...
  if(activeNavigationBooking)return <div style={{position:"relative",width:"100vw",height:"100dvh"}}><DriverNavigationMode booking={activeNavigationBooking} driverLocation={driverLocation} targetType={navigationTargetType} actionLabel={stepFor(activeNavigationBooking)?.label||"Continue"} notice={error} onDismissNotice={()=>setError("")} onExit={exitNavigation} onComplete={()=>advance(activeNavigationBooking)} overlay={actionSheet} /></div>;

  const active=bookings.filter((b)=>ACTIVE_STATUSES.includes(b.booking_status));
  const todayDate=todayIndia();
  const upcoming=bookings.filter((b)=>["driver_assigned","on_the_way","arrived","trip_started","waiting_for_return","return_trip_started"].includes(b.booking_status) && String(b.travel_date||"")>=todayDate && !commuteBookingIds.has(b.id)).sort((a,b)=>Number(ACTIVE_STATUSES.includes(b.booking_status))-Number(ACTIVE_STATUSES.includes(a.booking_status)));
