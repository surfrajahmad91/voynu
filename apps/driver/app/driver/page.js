...
export default function DriverPage() {
  ...
  const [bookings, setBookings] = useState([]), [loading, setLoading] = useState(false), [error, setError] = useState("");
  ...
  const fetchBookings = async()=>{ if(!driver)return; setLoading(true); const {data,error:e}=await supabase.from("bookings").select("*").eq("driver_id",driver.id).order("travel_date",{ascending:true}).order("pickup_time",{ascending:true}); setLoading(false); if(e){setError(e.message);return;} setBookings(data||[]); };
  useEffect(()=>{fetchBookings();},[driver]);
  useEffect(()=>{ if(!driver)return; const channel=supabase.channel(`voynu-driver-bookings-${driver.id}`).on("postgres_changes",{event:"*",schema:"public",table:"bookings",filter:`driver_id=eq.${driver.id}`},()=>fetchBookings()).subscribe(); const id=setInterval(fetchBookings,10000); return()=>{clearInterval(id);supabase.removeChannel(channel);}; },[driver]);
